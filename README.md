...
1. **pretrain**（`pretrain.py`）：在 teacher_train 上 next-token CE (只预测等号右边的内容)，**启用 CoT（`cot_fraction` > 0）** → `teacher_pretrain.pt`
2. **SFT**（`sft.py`）：从 teacher_pretrain 出发，数据集只用 CoT 的, 同样 loss 只在 target 部分 → `teacher_sft.pt`
3. **GRPO**（`grpo.py`）：从 teacher_sft 出发，verifier reward，→ `teacher_grpo.pt`
...
1. **pretrain**（`pretrain.py`）：在 student_train 上 next-token CE → `student_pretrain.pt`
...
├── visualize_llm.py    # 只读 model/*.pt, 可视化展示指定层的 attention 机制等等.
